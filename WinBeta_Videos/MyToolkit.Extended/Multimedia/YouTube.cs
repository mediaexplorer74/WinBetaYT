...
namespace MyToolkit.Multimedia
{
    ...
    public static class YouTube
    {
        ...
        public static async Task<YouTubeUri[]> GetUrisAsync(string youTubeId, CancellationToken token)
        {
            ...
            //RnD
            string response = await HttpGetAsync("https://www.youtube.com/watch?v=" + youTubeId + "&nomobile=1");
            var match = Regex.Match(response, "url_encoded_fmt_stream_map\": ?\"(.*?)\"");

            string data = response;//Uri.UnescapeDataString(match.Groups[1].Value);
            match = Regex.Match(response, "adaptive_fmts\": ?\"(.*?)\"");

            string data2 = "777";//response;//Uri.UnescapeDataString(match.Groups[1].Value);

            string[] arr = Regex.Split(data + "," + data2, ",");// (?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)"); // split by comma but outside quotes
